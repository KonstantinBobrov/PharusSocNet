    ...
    public int doStartTag() throws JspException {
        ...
        try {
            for (int i = 0; i < localMax; i++) {
                ...
                User user = DaoFactory.getInstance().getUserDao().getById(post.getUserId());
                html.append("<p><table border=1 width=100% class = 'tablistpost'>");
                html.append("<tr><td><p><b>");
                html.append(String.format("<a href='/user?id=%s'>", user.getId()));
                html.append(String.format("<div class='mainfriends' style=\"background-image:url('img/ava/%s')\"></div>", user.getAvatar()));

                html.append(String.format("<a href='/user?id=%s&view=%d'>%s %s</a>", user.getId(), post.getId(), bundle.getString("userpost"), user.getFullName()));
                html.append("</b>");
                html.append("</td></tr><tr><td><p>");
                html.append(list.get(i).getText());
                html.append("</td></tr>");
                if (logUser.getId() == user.getId() || adminMode) {
                    html.append("<tr><td>");
                    html.append(String.format(": <a href='/user?id=%s&delpost=%d'>%s</a>", user.getId(), post.getId(), bundle.getString("delete")));
                    html.append(String.format(": <a href='/user?id=%s&editpost=%d'>%s</a> :", user.getId(), post.getId(), bundle.getString("edit")));
                    html.append("</td></tr>");
                }
                html.append("</table>");
            }
